...
public class SOS implements CPU.TrapHandler 
{
    ...
    {	
		//search for the process in the vector of available processes
        int currID = m_currProcess.getProcessId();
        for (int i = 0; i < m_processes.size(); ++i)
        {
        	if((m_processes.elementAt(i)).getProcessId() == currID)
        	{
        		m_processes.remove(i);
        		break;
        	}
        }
        debugPrintln("Program Removed from RAM " + currID + " at " + m_CPU.getBASE());
